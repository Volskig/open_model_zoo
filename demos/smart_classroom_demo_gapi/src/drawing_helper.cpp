...
void DrawingHelper::GetNewFrameSize(const cv::Size& frame_size) {
    rect_scale_x_ = 1;
    rect_scale_y_ = 1;
    const auto max_width = max_input_width_;
    auto getOutputSize =
        [&max_width](const cv::Size& input_size) -> cv::Size {
            if (input_size.width > max_width) {
                float ratio = static_cast<float>(input_size.height) / input_size.width;
                return cv::Size(max_width, cvRound(ratio * max_width));
            }
            return input_size;
        };
